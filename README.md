...
[![Release](https://github.com/hacker65536/phd/actions/workflows/release.yml/badge.svg)](https://github.com/hacker65536/phd/actions/workflows/release.yml)
...
[Releases](https://github.com/hacker65536/phd/releases) から、自分の環境に合う
...
要約表の `STATUS` は occurrence の件数内訳（`open:2 upcoming:5`）、`NEXT` は直近の予定開始までの残り、`OCC` は配下 occurrence 数、`TOPIC`/`EVENT_TYPE` がファミリー名。各層の意味は [抽象化の4層](#抽象化の4層) を参照。
